...
func ResourceUPNPSettings() *schema.Resource {
	resSchema := map[string]*schema.Schema{
		...
		"allow_disable_external_interface": {
			...
			Optional: true,
			Description: "Whether or not should the users be allowed to disable the router's external interface. " +
		...
		"show_dummy_rule": {
			Type:     schema.TypeBool,
			Optional: true,
			Description: "Enable a workaround for some broken implementations, which are handling the absence of " +
